...
To replicate the results, run the script `main.R` after updating the working directory path on line 12 of `main.R`.
...
```
- The three `.csv` files: `IRF_Traeger_5y.csv`, `RCP_Mat_ACE.csv`, and `RCP_Mat_MAGICC.csv` are based on the [replication codes associated with Traeger (2023)](https://www.openicpsr.org/openicpsr/project/154141/version/V1/view). These files can be generated by sourcing the script `make_csv_files.m` in Matlab; this script is in the folder `data/csv_from_Traeger/`. Before sourcing `make_csv_files.m`, one has to download and save (in `data/csv_from_Traeger/`) two data files from Traeger's package, namely:
    - `impulse_timestep_5_logi_001.mat` directly available [here](https://www.openicpsr.org/openicpsr/project/154141/version/V1/view?path=/openicpsr/154141/fcr:versions/V1/impulse_timestep_5_logi_001.mat&type=file), and
    - `MagiccOcean.mat`, directly available  [here](https://www.openicpsr.org/openicpsr/project/154141/version/V1/view?path=/openicpsr/154141/fcr:versions/V1/MagiccOcean.mat&type=file).
