# ==============================================================================
# Figure showing term structure of Temperat.-Indexed-Bond yields
# ==============================================================================
...
T.swaps<-varphi.tilde(model_sol,omega_T.at,H)[["P.tilde"]]/
          varphi(model_sol,omega_ZCB,H)[["P.t"]]
...
par(mfrow=c(2,1))
plot(model_sol$vec_date[2:(H+1)],EV$EX$T_at[1:H],
     type="l",las=1,
     xlab="Maturity",
...
lines(model_sol$vec_date[2:(H+1)],T.swaps,
      col=Q.col.line,
      lwd=2)
legend("topleft",
...
plot(model_sol$vec_date[2:(H+1)],v.0$r.t,
     type="l",
     ...
     cex.main=1.2,cex.axis=1.2,cex.lab=1.2)
for(i in 1:length(chi)){
  lines(model_sol$vec_date[2:(H+1)],tib[[i]]$r.tib,
        col=Q.col.line,
        lty=lty.tib[i],
        lwd=2)
}
